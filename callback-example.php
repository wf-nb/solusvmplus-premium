...
switch ($action) {
	case "suspend":
		...
		Capsule::table('tblhosting')->where('id', $hosting_id)->update(['domainstatus' => 'Suspended', 'suspendreason' => $extra_var["suspendreason"],]);
		...
		break;
	case "unsuspend":
		Capsule::table('tblhosting')->where('id', $hosting_id)->update(['domainstatus' => 'Active', 'suspendreason' => '',]);
		echo "<success>1</success>";
		break;
	case "terminate":
	...
	case "changerootpassword":
		$rootValueID = Capsule::table('tblcustomfields')->join('tblcustomfieldsvalues', 'tblcustomfieldsvalues.fieldid', '=', 'tblcustomfields.id')->select('tblcustomfieldsvalues.fieldid as rootID')->where('tblcustomfields.type', 'product')->where('tblcustomfieldsvalues.relid', $product->value_productid)->where('tblcustomfields.fieldname', 'rootpassword')->first();
		Capsule::table('tblcustomfieldsvalues')->where('relid', $hosting_id)->where('fieldid', $rootValueID->rootID)->update(['value' => $extra_var['newrootpassword'],]);
		echo "<success>1</success>";
		break;
	default:
		echo "<success>1</success>";
